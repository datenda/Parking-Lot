...
export default function ExternasPage() {
  ...
  const handleEstado = async (matricula, estado) => {
    try {
      ...
      if (fetchLugar.status === 200) {
        ...
        const { selectedGalera, lugar } = jsonResponse;
        setFilteredItems((prevFilteredItems) => {
          return prevFilteredItems.map((item) => {
        ...
        console.log(lugar);
        showToast("success", `A galera está no parque teste${lugar.lugar}`);
      ...
      console.error("Error fetching data:", error);
      showToast("error", "O estado atual é o mesmo");
    }
  };
