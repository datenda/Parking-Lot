...
export default function GalerasPage() {
  ...
  const [isSelectedFrio, setIsSelectedFrio] = useState();
  const [isSelectedMat, setIsSelectedMat] = useState();
  ...
  let list = useAsyncList({
    async load({ signal }) {
      try {
        let res = await fetch(
          "https://tfs-server-1.onrender.com/api/galera/oficina",
          {
            method: "GET",
            signal,
            headers: {
  ...
  const handleEstado = async (matricula, estado) => {
    try {
      ...
      const requestBody = {
        matricula: matricula,
        status: estado,
        ...(isSelectedFrio !== undefined && { frio: isSelectedFrio }),
        ...(isSelectedMat !== undefined && { mat: isSelectedMat }),
        op: "oficina",
      };
      ...
      if (fetchLugar.status === 200) {
      ...
      } else if (fetchLugar.status === 202) {
        setFilteredItems((prevFilteredItems) => {
          return prevFilteredItems.map((item) => {
            if (item.matricula === jsonResponse.selectedGalera.matricula) {
              console.log("202", jsonResponse);
              return jsonResponse.selectedGalera;
            }
            return item;
          });
        });
        showToast(
          "success",
          `A galera está no parque ${jsonResponse.lugar.lugar}`
        );
      } else if (fetchLugar.status === 400) {
        showToast("danger", "A galera já esta no parquimetro");
      }
      console.log(fetchLugar.status);
      setIsSelectedMat();
      setIsSelectedFrio();
    } catch (error) {
      console.error("Error fetching data:", error);
      showToast("error", "O estado atual é o mesmo ou já está no parque");
    }
  };
  ...
  const handleRowClick = (itemId) => {
    ...
    if (selected) {
      ...
      setIsSelectedFrio(selected.frio);
      setIsSelectedMat(selected.mat);
    }
  };
                  ...
                  <div className="text-text">
                    {modalItem ? (
                      <div>
                        ...
                        {"mat" in modalItem && (
                          <Checkbox
                            isSelected={isSelectedMat}
                            onValueChange={setIsSelectedMat}
                            className="mt-2"
                          >
                            <div className="text-text">Manutenção</div>
                          </Checkbox>
                        )}
                        {"frio" in modalItem && (
                          <div className="flex-col flex">
                            <Checkbox
                              isSelected={isSelectedFrio}
                              onValueChange={setIsSelectedFrio}
                              className="mt-2"
                            >
                              <div className="text-text"> Frio </div>
                            </Checkbox>
                          </div>
                        )}
                      </div>
